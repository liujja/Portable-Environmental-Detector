...
#define MLX90640_FRAME_TIMEOUT_ERROR   (-1001)
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        ...
        {
            return MLX90640_FRAME_TIMEOUT_ERROR;
        }
    }      
    
    /* PRESERVE status register bits (e.g., subpage) and ONLY clear Data Ready bit (Bit 3) */
    error = MLX90640_I2CWrite(slaveAddr, MLX90640_STATUS_REG, statusRegister & ~0x0008);
    if(error == -MLX90640_I2C_NACK_ERROR)
    {
        return error;
    }
...
{
    static float vdd_offset = 0;
    static uint8_t vdd_cal = 0;
    float vdd;
    ...
    if (!vdd_cal)
    {
        /* First frame: assume supply is 3.3V, compute offset */
        vdd_offset = vdd - 3.3f;
        vdd_cal = 1;
    }

    return vdd - vdd_offset;
}
...
{
    static float ta_offset = 0;
    static uint8_t ta_cal = 0;
    int16_t ptat;
    float ptatArt;
    float vdd;
    float ta;
    ...
    if (!ta_cal)
    {
        /* First frame: assume ambient ~25C, compute offset */
        ta_offset = ta - 25.0f;
        ta_cal = 1;
    }
