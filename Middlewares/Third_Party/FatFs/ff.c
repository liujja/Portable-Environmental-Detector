...
{
	static FRESULT res;
	static UINT i, sz_ent;
	static BYTE *dirb;
  dirb = dp->obj.fs->dirbuf;	/* Pointer to the on-memory direcotry entry block 85+C0+C1s */
...
)
{
	...
	static UINT nent;
	static BYTE *dirb;
  dirb = dp->obj.fs->dirbuf;	/* Pointer to the direcotry entry block 85+C0+C1s */
...
)
{
  ...
  fs = dp->obj.fs;
#if FF_USE_LFN		/* LFN configuration */
	static UINT n, len, n_ent;
	static BYTE sn[12], sum;
...
)
{
	...
	static DSTATUS stat;
	static LBA_t bsect;
	static DWORD tsect, sysect, fasize, nclst, szbfat;
	static WORD nrsv;
	static FATFS *fs;
	static UINT fmt;
...
)
{
	...
	*bw = 0;	/* 清除写字节计数器 */
	...
	if (res != FR_OK || (res = (FRESULT)fp->err) != FR_OK) LEAVE_FF(fs, res);	/* 检查有效性 */
	if (!(fp->flag & FA_WRITE)) LEAVE_FF(fs, FR_DENIED);	/* 检查访问模式 */

	/* 检查 fptr 环绕（文件大小不能达到 4 GiB 在 FAT 卷） */
	...
	{
		if (fp->fptr % SS(fs) == 0) /* 在扇区边界上? */
		{		
			csect = (UINT)(fp->fptr / SS(fs)) & (fs->csize - 1);	/* 簇中的扇区偏移 */
			if (csect == 0) /* 在簇边界上? */
			{				
				if (fp->fptr == 0) /* 在文件的顶部? */
				{		
					clst = fp->obj.sclust;	/* 从初始位置跟随 */
					if (clst == 0) /* 如果簇没有被分配 */
					{		
						clst = create_chain(&fp->obj, 0);	/* 创建一个新的簇链 */
					}
				} 
				else /* 在文件的中间或末尾 */
				{					
...
#endif
					{
						clst = create_chain(&fp->obj, fp->clust);	/* 跟随或拉伸 FAT 上的簇链 */
					}
				}
				if (clst == 0) break;		/* 无法分配新簇（磁盘已满） */
				if (clst == 1) ABORT(fs, FR_INT_ERR);
				if (clst == 0xFFFFFFFF) ABORT(fs, FR_DISK_ERR);
				fp->clust = clst;			/* 更新当前簇 */
				if (fp->obj.sclust == 0) fp->obj.sclust = clst;	/* 如果是第一次写入,则设置起始簇 */
			}
#if FF_FS_TINY
			if (fs->winsect == fp->sect && sync_window(fs) != FR_OK) ABORT(fs, FR_DISK_ERR);	/* 写回扇区缓存 */
#else
			...
			if (sect == 0) ABORT(fs, FR_INT_ERR);
			sect += csect;
			cc = btw / SS(fs);				/* 当剩余字节数>=扇区大小, */
			if (cc > 0) /* 字节写入最大连续扇区 */
			{
				if (csect + cc > fs->csize) /* 在簇边界剪裁 */
				{	
					cc = fs->csize - csect;
				}
...
#if FF_FS_MINIMIZE <= 2
#if FF_FS_TINY
				if (fs->winsect - sect < cc) /* 如果字节写入无效,则重新填充扇区缓存 */
				{	
...
#if FF_FS_TINY
			if (fp->fptr >= fp->obj.objsize)/* 避免在不断增长的边缘进行愚蠢的缓存填充 */
			{	
		...
		wcnt = SS(fs) - (UINT)fp->fptr % SS(fs);	/* 扇区中剩余的字节数 */
		if (wcnt > btw) wcnt = btw;					/* 如果需要,顺便把它剪下来 */
#if FF_FS_TINY
		if (move_window(fs, fp->sect) != FR_OK) ABORT(fs, FR_DISK_ERR);	/* 移动扇区窗口 */
		
		memcpy(fs->win + fp->fptr % SS(fs), wbuff, wcnt);	/* 将数据合并到扇区 */
		
		fs->wflag = 1;
#else
		memcpy(fp->buf + fp->fptr % SS(fs), wbuff, wcnt);	/* 将数据合并到扇区 */
		fp->flag |= FA_DIRTY;
#endif
	}
...
)
{
	static FRESULT res;
	static FATFS *fs;
	static DWORD clst, bcs;
	static LBA_t nsect;
	static FSIZE_t ifptr;
#if FF_USE_FASTSEEK
...
)
{
	...
	static DIR djo, djn;
	static FATFS *fs;
	static BYTE buf[FF_FS_EXFAT ? SZDIRE * 2 : SZDIRE], *dir;
	static LBA_t sect;
	DEF_NAMBUF
...
)
{
	static UINT i, cy;
	static LBA_t sz_drv;
	static DWORD sz_drv32, nxt_alloc32, sz_part32;
	static BYTE *pte;
	static BYTE hd, n_hd, sc, n_sc;
...
)
{
	...
	static BYTE fsopt, fsty, sys, *buf, *pte, pdrv, ipart;
	static WORD ss;	/* Sector size */
	static DWORD sz_buf, sz_blk, n_clst, pau, nsect, n, vsn;
	static LBA_t sz_vol, b_vol, b_fat, b_data;		/* Size of volume, Base LBA of volume, fat, data */
	static LBA_t sect, lba[2];
	static DWORD sz_rsv, sz_fat, sz_dir, sz_au;	/* Size of reserved, fat, dir, data, cluster */
	static UINT n_fat, n_root, i;					/* Index, Number of FATs and Number of roor dir entries */
	static int vol;
	static DSTATUS ds;
	static FRESULT fr;
